...
这道题是综合题，通常用来判断候选人能否把浏览器、网络、安全、缓存、渲染串起来。回答时不要陷入每个细节，而要先给主链路，再根据追问展开。
...
这个流程本质上是浏览器把“用户输入的地址”转换成“屏幕上的像素”的过程。它跨越了浏览器主进程、网络进程、渲染进程和 GPU 进程，也跨越了缓存、DNS、TCP、TLS、HTTP、HTML/CSS/JS 解析、布局、绘制与合成。
...
不一定。浏览器、操作系统、路由器、本地 DNS、甚至应用层都可能有 DNS 缓存；如果缓存命中，就不需要完整递归查询。

### 输入 URL 后一定会发 HTTP 请求吗？

不一定。强缓存命中、Service Worker 拦截、浏览器本地页面、非法协议或安全策略拦截，都可能不发真实网络请求。

### DOMContentLoaded 和 load 区别？

`DOMContentLoaded` 表示 DOM 构建完成，不等待图片等资源全部加载。`load` 表示页面依赖资源基本加载完成。
