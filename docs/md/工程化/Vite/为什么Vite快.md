...
这是 Vite 高频面试题。回答时要抓住开发环境和生产环境的不同目标：开发环境追求启动和 HMR 快，生产环境追求产物体积和加载性能。
...
> Vite 快主要快在开发阶段。传统 Webpack dev server 启动时要先从入口递归构建依赖图并打包，项目越大启动越慢。Vite 开发环境利用浏览器原生 ESM，不做全量打包，只启动一个 dev server，浏览器请求哪个模块，Vite 才按需转换哪个模块。第三方依赖会先用 esbuild 预构建，把 CommonJS/UMD 转成 ESM，同时把内部模块很多的依赖合并，减少浏览器请求数。HMR 时 Vite 只需要让浏览器重新请求变更模块和相关边界，不需要重新打包整个项目。生产环境 Vite 仍然使用 Rollup 打包，因为线上更关注请求数量、Tree Shaking、代码分割和压缩优化。

## 高频追问

### Vite 为什么开发环境不打包？

因为现代浏览器已经支持 ESM。开发时按需编译能避免启动阶段全量构建，改一个文件也只处理相关模块。

### Vite 为什么生产环境还要打包？

如果生产环境直接用大量源码 ESM，会产生很多网络请求，也缺少完整 Tree Shaking、压缩、代码分割等优化。生产环境需要打包成更适合线上加载的产物。

### Vite 一定比 Webpack 好吗？

不一定。Vite 开发体验通常更快，但 Webpack 生态成熟、可定制能力强，老项目和复杂构建场景仍可能更适合 Webpack。
