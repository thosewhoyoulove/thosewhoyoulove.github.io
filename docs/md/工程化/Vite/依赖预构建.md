...
依赖预构建是解释 Vite 快的关键点。面试重点是：为什么要预构建、预构建处理了 CommonJS 兼容和请求数量问题、缓存如何失效。
...
> Vite 的依赖预构建主要解决两个问题。第一是兼容性，开发环境 Vite 把源码当作原生 ESM 处理，但很多 npm 包仍然是 CommonJS 或 UMD，需要先用 esbuild 转成 ESM。第二是性能，有些 ESM 依赖内部文件非常多，比如 lodash-es 可能引发大量浏览器请求，Vite 会把这些依赖预构建成更少的模块，减少请求数量。预构建结果缓存在 `node_modules/.vite`，lockfile、vite 配置、环境等变化时会失效，也可以用 `--force` 强制重建。

## 高频追问

### optimizeDeps.include 和 exclude 什么时候用？

当 Vite 没能在初始扫描中发现某个依赖，导致启动后重新预构建，可以用 include 提前包含。某些已经是标准 ESM 且不需要合并的依赖，可以用 exclude 排除。

### Monorepo 里的本地包会被预构建吗？

Vite 通常会把链接依赖当作源码处理，不直接预构建。如果本地包不是 ESM 或依赖处理异常，需要配合 `optimizeDeps.include` 和 `build.commonjsOptions.include`。
