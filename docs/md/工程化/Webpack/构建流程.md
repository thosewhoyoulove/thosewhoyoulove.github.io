...
Webpack 构建流程题考察你是否理解打包工具内部工作方式。回答重点是 Compiler、Compilation、Loader、Plugin、依赖图、Chunk 和 Emit。
...
> Webpack 构建可以理解为从 entry 出发递归构建依赖图，最后生成资源。启动时 Webpack 会读取配置、合并 CLI 参数、创建 Compiler 实例并注册插件。进入编译后创建 Compilation，从入口模块开始解析源码，遇到不同类型文件时先经过 loader 转换成 JavaScript 可理解的模块，再分析 import/require 继续递归构建依赖图。依赖图构建完成后，Webpack 根据入口、动态导入和 splitChunks 生成 chunk，再把 chunk 转成最终 assets，最后在 emit 阶段输出到磁盘。Plugin 则通过 Tapable 钩子介入这些生命周期，影响构建过程和输出结果。

## 高频追问

### Compiler 和 Compilation 区别？

Compiler 代表一次完整 Webpack 运行环境，包含配置、插件和生命周期。Compilation 代表一次具体编译过程，包含模块、依赖、chunk、asset 等信息。watch 模式下一个 Compiler 会产生多次 Compilation。

### Loader 和 Plugin 区别？

Loader 处理单个模块，把非 JS 或新语法转换成 Webpack 能处理的模块；Plugin 通过生命周期钩子介入整个构建流程，能力更广。

### Webpack 如何处理动态 import？

动态 import 会形成异步 chunk，Webpack 在构建时生成单独文件，运行时按需加载。
